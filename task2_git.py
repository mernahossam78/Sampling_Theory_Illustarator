...
def sine_wave():
    ...
    global wave
    global t
    wave = np.sin((w * t)-phase_shift ) * amplitude
    ax1.cla()
    ax1.plot(t, wave)
    ax1.set_xlim(0,max(t))
    ax1.set_ylim(-max(wave)-1,max(wave)+1)
...
def my_remove_sel():
    list_index=list2.index(value_inside.get())
    list1.pop(list_index)
    list2.pop(list_index)
    r_index=question_menu['menu'].index(value_inside.get())
    question_menu['menu'].delete(r_index)
    value_inside.set(question_menu['menu'].entrycget(0,"label"))
    dropupdater()
    plotting()
    


def first_sin(signal1, signal2):
    global w
    global phase_shift
    global amplitude
    global t
